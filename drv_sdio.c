...
static void rthw_sdio_wait_completed(struct rthw_sdio *sdio)
{
    ...
    if (rt_event_recv(&sdio->event, 0xffffffff, RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR, rt_tick_from_millisecond(5000),
            &status) != RT_EOK)
    {
    ...
    if (status & HW_SDIO_ERRORS)
    {
        ...
        {
            LOG_D("err:0x%08x, %s%s%s%s%s%s%s cmd:%d arg:0x%08x rw:%c len:%d blksize:%d", status,
                    status & HW_SDIO_IT_CCRCFAIL ? "CCRCFAIL " : "", status & HW_SDIO_IT_DCRCFAIL ? "DCRCFAIL " : "",
                    status & HW_SDIO_IT_CTIMEOUT ? "CTIMEOUT " : "", status & HW_SDIO_IT_DTIMEOUT ? "DTIMEOUT " : "",
                    status & HW_SDIO_IT_TXUNDERR ? "TXUNDERR " : "", status & HW_SDIO_IT_RXOVERR ? "RXOVERR " : "",
                    status == 0 ? "NULL" : "", cmd->cmd_code, cmd->arg,
                    data ? (data->flags & DATA_DIR_WRITE ? 'w' : 'r') : '-', data ? data->blks * data->blksize : 0,
                    data ? data->blksize : 0);
        }
    }
    else
...
static void rthw_sdio_send_command(struct rthw_sdio *sdio, struct sdio_pkg *pkg)
{
    ...
    LOG_D("CMD:%d ARG:0x%08x RES:%s%s%s%s%s%s%s%s%s rw:%c len:%d blksize:%d", cmd->cmd_code, cmd->arg,
            resp_type(cmd) == RESP_NONE ? "NONE" : "", resp_type(cmd) == RESP_R1 ? "R1" : "",
            resp_type(cmd) == RESP_R1B ? "R1B" : "", resp_type(cmd) == RESP_R2 ? "R2" : "",
            resp_type(cmd) == RESP_R3 ? "R3" : "", resp_type(cmd) == RESP_R4 ? "R4" : "",
            resp_type(cmd) == RESP_R5 ? "R5" : "", resp_type(cmd) == RESP_R6 ? "R6" : "",
            resp_type(cmd) == RESP_R7 ? "R7" : "", data ? (data->flags & DATA_DIR_WRITE ? 'w' : 'r') : '-',
            data ? data->blks * data->blksize : 0, data ? data->blksize : 0);
...
static void rthw_sdio_iocfg(struct rt_mmcsd_host *host, struct rt_mmcsd_io_cfg *io_cfg)
{
    ...
    LOG_D("clk:%d width:%s%s%s power:%s%s%s", clk, io_cfg->bus_width == MMCSD_BUS_WIDTH_8 ? "8" : "",
            io_cfg->bus_width == MMCSD_BUS_WIDTH_4 ? "4" : "", io_cfg->bus_width == MMCSD_BUS_WIDTH_1 ? "1" : "",
            io_cfg->power_mode == MMCSD_POWER_OFF ? "OFF" : "", io_cfg->power_mode == MMCSD_POWER_UP ? "UP" : "",
...
struct rt_mmcsd_host *sdio_host_create(struct stm32_sdio_des *sdio_des)
{
    ...
    {
        LOG_E("L:%d F:%s %s %s %s", (sdio_des == RT_NULL ? "sdio_des is NULL" : ""),
    ...
    sdio->sdio_des.hw_sdio =
            (sdio_des->hw_sdio == RT_NULL ? (struct stm32_sdio *) SDIO_BASE_ADDRESS : sdio_des->hw_sdio);
...
/*  Clear all interrupt flags and set dma status to ready
 *  Change Logs:
 *  Date: 2025/03/06
 *  Author: zj
 * */
static void sdio_dma_reset_status(DMA_HandleTypeDef *dma_handle)
{
    ...
    regs->IFCR = 0x3FU << dma_handle->StreamIndex;
}
...
void SD_LowLevel_DMA_TxConfig(uint32_t *src, uint32_t *dst, uint32_t BufferSize)
{
    ...
    sdio_dma_reset_status(&sdio_obj.dma.handle_tx);
...
{
    /* Clear all interrupt flags and set dma status to ready*/
    sdio_dma_reset_status(&sdio_obj.dma.handle_rx);
...
void SD_DMA_StaticConfig(void)
{
    ...
    /* DMA_PFCTRL */
    if (HAL_DMA_Init(&sdio_obj.dma.handle_tx) != HAL_OK)
    {
        rt_kprintf("dma init tx faild\n");
    }

    //sdio_obj.cfg = &sdio_config;
    sdio_obj.dma.handle_rx.Instance = sdio_config.dma_rx.Instance;
    sdio_obj.dma.handle_rx.Init.Channel = sdio_config.dma_rx.channel;
    ...
    if (HAL_DMA_Init(&sdio_obj.dma.handle_rx) != HAL_OK)
    {
        rt_kprintf("dma init rx faild\n");
    }
}
